...
You've spotted one such source, and you think solving it will help you 
build the reputation you need to get promoted.
...
You think that by removing the salute requirement, you could 
save several collective hours of employee time per day. 
...
typical walk along a hallway. The hall is represented by a string. 
For example: "--->-><-><-->-"
...
right or to the left, according to their direction. Whenever two employees cross, 
...
string=[ "<<>><", "--->-><-><-->-", ">----<" ]
for s in string:
	print("number of salutes: ", count_salutes(s))
	number_salutes(s)
